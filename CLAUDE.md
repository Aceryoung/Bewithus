...
## ⚠️ 절대 금지 사항 (프로덕션 데이터 보호)

- `supabase db reset` 명령어 실행 금지
- 데이터베이스 초기화/리셋 관련 명령어 실행 전 반드시 사용자 확인 필요
- 실제 운영 DB(Supabase 클라우드)에 직접 DELETE/TRUNCATE/DROP 실행 금지
- 마이그레이션 오류 해결 시 리셋 대신 수정 마이그레이션 파일 작성
